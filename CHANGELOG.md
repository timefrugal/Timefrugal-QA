...
### Changed
- **AI review backend switched from GitHub Models to Groq** — GitHub fully retired GitHub Models (playground, catalog, inference API, BYOK) on 2026-07-30 for every customer, no exceptions (brownouts ran 2026-07-16/23 as warning). Every PR reaching the AI-review step (any PR touching real code, not just docs/YAML) had started failing the whole check with a 401/410 depending on which of the two Models endpoints got hit — first surfaced via a consumer repo (jarvis-infra, PR #63), which was the first real-code PR to run since the deprecation began, so it had gone undetected until then. `qa_agent` now uses [Groq](https://console.groq.com)'s free tier (`https://api.groq.com/openai/v1`, OpenAI-compatible, same client code) with `llama-3.3-70b-versatile` as the default model, authenticated via a new `GROQ_API_KEY` env var/secret (replaces `GITHUB_TOKEN` for the AI-review call specifically; `GITHUB_TOKEN` is still used for posting PR comments/commit status). **Action required for every consumer repo**: add a `GROQ_API_KEY` secret (free at console.groq.com/keys) — without it, the AI-review step fails closed with a clear error rather than silently skipping, same as today's GitHub Models failure mode. `templates/repo_workflow.yml` and this repo's own `qa.yml` updated to pass it through; `models: read` permission removed (no longer needed). All docs (`README.md`, `CLAUDE.md`, `scripts/run_local_qa.sh`) updated to match.
