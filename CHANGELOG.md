...
### Fixed (this entry)
- **BLOCKED header hardcoded "Critical/High" no matter which cutoff actually fired** — `pr_reporter._build_comment()` always wrote `## 🔴 Timefrugal-QA — BLOCKED: Critical/High issues require attention before merge`, and `local_reporter.print_report()` always wrote `BLOCKED — Fix critical/high issues before raising a PR`, ignoring the per-run effective `block_merge_threshold` those same reports gate on. On any repo whose `.timefrugal-qa.yml` sets something stricter than the HIGH default, this is actively false: jarvis-infra (`block_merge_threshold: MEDIUM`, deliberate — it runs live production infra) got that header on PR #312 directly above its own summary table reading **0 Critical / 0 High**, and the contradiction was reasonably read as the gate being disconnected from its own severity counts (filed as jarvis-infra#323). Nothing was actually broken: 28 real MEDIUM pylint findings (W0102, W0718, W0613, W0603) legitimately tripped a MEDIUM cutoff. Only the wording lied.

  Fix: two shared helpers in `static_analysis.py` — `effective_block_threshold(results, ai_blocking)` resolves which cutoff actually gated the run, and `blocking_severity_label(threshold)` renders it (`Critical/High` for the HIGH default, `Medium-or-above`/`Low-or-above` for stricter repos, generic across all five `SEVERITY_ORDER` values rather than a fixed set of strings). Both reporters now render that instead of a literal. It takes `ai_blocking` as a **bool**, not an `AIReview`, so `static_analysis` gains no import of `ai_review` (which imports it). When both sources block at once — static at a repo-configured cutoff and AI at its own always-CRITICAL/HIGH one — the broadest cutoff wins, since that's the only one that explains every finding the gate reacted to; AI-only blocks on a stricter repo therefore still correctly read "Critical/High". `pr_reporter` additionally emits a one-line note under the header whenever the cutoff is stricter than the default, naming the severity and whether it came from `.timefrugal-qa.yml` or `QA_BLOCK_MERGE_THRESHOLD`, so the answer to "why is a Medium blocking me?" is in the report rather than in the tool's source.

  Deliberately NOT changed: `AnalysisResults.has_blocking_issues` and `AIReview.has_blocking_issues` (both were already correct — this was a reporting bug only, and the gate's actual pass/fail decision is byte-for-byte unchanged for every repo); the complexity-findings-are-advisory carve-out; and `set_commit_status`'s description text, which never named a severity in the first place. Default-threshold repos see zero wording change.

  Regression coverage: `tests/test_static_analysis.py`'s `TestEffectiveBlockThresholdAndLabel` covers the helpers (default HIGH, configured MEDIUM/CRITICAL/LOW, both-sources-blocking, AI-only on a MEDIUM repo, nothing-blocking, complexity findings still never triggering). `tests/test_pr_reporter.py`'s `TestBuildCommentBlockedHeader` drives the real `_build_comment()` — first tests of that function — asserting the default header is unchanged, that a MEDIUM-cutoff block with 0 Critical / 0 High names MEDIUM and explains itself, that AI-only blocking still says Critical/High, that the passing path is untouched, and that the blocked+errored suffix survived the f-string refactor. Mutation check: restoring the hardcoded header string fails `test_medium_threshold_block_with_zero_critical_high_names_medium`.
