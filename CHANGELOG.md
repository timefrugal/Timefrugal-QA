...
### Fixed
- **AI review always errored with a 401 on real code changes** — `GITHUB_MODELS_BASE_URL` pointed at `https://models.inference.ai.azure.com`, GitHub's deprecated Models endpoint. Any run that actually reached the AI-review step (i.e. any PR touching real code, not just docs/YAML) failed the whole check with `{"error":{"code":"unauthorized","message":"Server Error"}}`, even though static analysis and the `GITHUB_TOKEN`/`models: read` permission were both fine — the token worked, the endpoint was just gone. Repointed to the current endpoint, `https://models.github.ai/inference`; confirmed against the live API with the same token/model that had been failing. Found via a consumer repo (jarvis-infra) where this was the first real-code PR to run since the endpoint's deprecation, so it had gone undetected.
